...
class Node(MCTSNODE):
    ...
    def backpropagate(self, result):
        self.visits += 1
        self.score += result
        if self.visits>len(self.possibleMoves)*3:
            self.cutoff=True
        if self.parent:
    ...
    def calculate_score(self,node,totalVisits):
        if self.cutoff:
            ...
            return (node.score + node.amaf_score.get(node.move, 0)) / (1 + node.visits) + self.C*math.sqrt( math.log(totalVisits) / (1 + node.visits))
